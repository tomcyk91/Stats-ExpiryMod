...
namespace StatisticMod
{
    internal static partial class ModLocalization
    {
        ...
        public static string ProductName(int productId, ProductSO fallback = null)
        {
            try
            {
                ...
                if (manager != null)
                {
                    ...
                    if (IsUsableProductName(localized))
                        return localized.Trim();
                }
            }
            catch { }

            // If the game localization system returns a placeholder such as
            // "No translation found for '_custom_key'", use the product's own
            // name instead. This is especially important for custom/modded products.
            if (fallback != null)
            {
                try
                {
                    if (IsUsableProductName(fallback.TempProductName))
                ...
                try
                {
                    if (IsUsableProductName(fallback.ProductName))
                ...
                try
                {
                    ...
                    if (IsUsableProductName(complex))
                        return complex.Trim();
                }
                catch { }
            }
        ...
        private static bool IsUsableProductName(string value)
        {
            ...
            // Unity/game localization placeholder shown when a localization key
            // does not exist. Never display it as a product name.
            if (text.StartsWith("No translation found", StringComparison.OrdinalIgnoreCase))
                return false;

            if (text.IndexOf("translation not found", StringComparison.OrdinalIgnoreCase) >= 0)
                return false;
