    ...
    public static class BusinessAnalysisService
    {
        public static List<ProductBusinessAnalysisRow> BuildRows(int rangeDays)
        {
            ...
            foreach (var pair in map)
            {
                ...
                CalculateDemandForecast(row, lastClosedDay);
        ...
        private static void CalculateDemandForecast(ProductBusinessAnalysisRow row, int analysisEndDay)
        {
            if (row == null) return;

            float recent = row.RecentAverageVisible;
            float longAverage = GetAverageDemand(
                row.ProductId,
                Mathf.Max(1, analysisEndDay - 13),
                analysisEndDay,
                row.IsWeight,
                row.KgPerUnit);
            ...
            if (forecastVisible <= 0.0001f)
                forecastVisible = row.AverageDailyDemandVisible;

            row.ForecastDailyVisible = Mathf.Max(0f, forecastVisible);
            row.Forecast3DayVisible = row.ForecastDailyVisible * 3f;
        }

        private static void CalculateRestock(ProductBusinessAnalysisRow row, int analysisEndDay)
        {
            // Prognoza jest liczona wcześniej dla wszystkich produktów, także tych,
            // które nie korzystają ze zwykłego systemu półek (np. lody). Dzięki temu
            // widok POPYT i rekomendacje UZUPEŁNIANIA używają tego samego wyniku.
            float forecastVisible = row.ForecastDailyVisible;
            if (forecastVisible <= 0.0001f)
            {
                CalculateDemandForecast(row, analysisEndDay);
                forecastVisible = row.ForecastDailyVisible;
            }
        ...
        private static void CalculatePricing(ProductBusinessAnalysisRow row)
        {
            ...
            // v15: the player's current base price is the starting point.
            // MarketPrice is a reference/floor for permanent changes, not an
            // automatic target. Falling demand is handled by a temporary promo.
            float suggestion = row.CurrentPrice > 0f
                ? row.CurrentPrice
                : (row.MarketPrice > 0f ? row.MarketPrice : row.CurrentCost * 1.05f);
            ...
            else if (row.CurrentPrice > 0f &&
                     row.MarketPrice > 0f &&
                     row.CurrentPrice < row.MarketPrice * 0.98f)
            {
                // The player's base price is materially below the game's optimum.
                suggestion = row.MarketPrice;
                advice = PricingAdviceType.RaiseSlightly;
            }
            else if (row.DemandTrend >= 0.15f &&
                     row.ServiceLevel >= 0.95f &&
                     row.DaysOfCover >= 2f &&
                     row.DaysOfCover <= 5f)
            {
                suggestion = row.CurrentPrice * 1.03f;
                advice = PricingAdviceType.RaiseSlightly;
            }
            else if (row.DemandTrend <= -0.15f && row.DaysOfCover > 5f)
            {
                // Signal for Desktop Insights: suggest a temporary promotion,
                // but keep the permanent selling price unchanged.
                suggestion = row.CurrentPrice;
                advice = PricingAdviceType.LowerSlightly;
            }

            float costFloor = row.CurrentCost > 0f
                ? row.CurrentCost * 1.05f
                : 0f;
            float marketFloor = row.MarketPrice > 0f
                ? row.MarketPrice
                : 0f;
            float minimum = Mathf.Max(costFloor, marketFloor);

            float maximum = 0f;
            try
            {
                if (Plugin.ProductCache != null &&
                    Plugin.ProductCache.TryGetSO(row.ProductId, out ProductSO so) &&
                    so != null &&
                    so.MaxProfitRate > 0f)
                {
            ...
            if (maximum <= minimum)
                maximum = Mathf.Max(minimum, suggestion * 1.20f);

            if (advice == PricingAdviceType.RaiseSlightly)
                row.SuggestedPrice = RoundPrice(Mathf.Clamp(suggestion, minimum, maximum));
            else
                row.SuggestedPrice = RoundPrice(row.CurrentPrice > 0f ? row.CurrentPrice : suggestion);
