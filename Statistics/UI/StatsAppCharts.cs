...
namespace StatisticMod
{
    ...
    public partial class StatsAppManager
    {
        ...
        private void UpdateCategoryCycleLabel()
        {
            ...
            AttachChartMetricTooltip(_categoryCycleLabel);
        }
        ...
        private string FormatMetricValue(Metric m, float val, AmountUnit unit)
        {
            ...
            if (m == Metric.SoldRevenue || m == Metric.WasteLoss)
                return Plugin.Money(val, 2, true);
        ...
        private void DrawDayLabel(float xCenter, int day, float groupW, float y = 8f)
        {
        ...
        private void UpdateLegendSingleMetric(AmountUnit unit)
        {
            ...
            bool moneyMetric = _chartMetric == Metric.SoldRevenue || _chartMetric == Metric.WasteLoss;
            string unitStr = unit == AmountUnit.Kg ? "kg" : pcsLabel;

            string txt;
            if (moneyMetric)
            {
                txt = $"{metricName}: <color={hexCol}>{Plugin.Money(totalValue, 2, true)}</color>";
            }
            else
            {
                string formattedTotal = unit == AmountUnit.Kg
                    ? totalValue.ToString("N2")
                    : totalValue.ToString("N0");
                txt = $"{metricName} ({unitStr}): <color={hexCol}>{formattedTotal}</color>";
            }
