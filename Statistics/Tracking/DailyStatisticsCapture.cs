    ...
    public static class DailyStatisticsScreen_ApplyStatistics_Patch
    {
            ...
            float __16, float __17, float __18, float __19)
        {
            try
            {
                ...
                // v14.3: confirmed day-end path on game v1.6.0.
                // The uploaded log proves this Prefix fires because the
                // [DailySummary] Captured day=... line is written below.
                try
                {
                    if (SmartExpiration.PluginConfig.ExpiryEnabled)
                    {
                        SmartExpiration.Patches.DailySpoilagePatches.RunCleanupForDay(
                            day,
                            "DailyStatisticsScreen.ApplyStatistics");
                    }
                }
                catch (Exception cleanupEx)
                {
                    Plugin.Log?.LogWarning(
                        "[Nocne Sprzątanie] Direct cleanup failed: " +
                        cleanupEx.Message);
                }
