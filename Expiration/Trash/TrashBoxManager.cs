        ...
        static bool Prefix(BoxInteraction __instance)
        {
            try
            {
                ...
                if (trash != null)
                {
                    ...
                    if (TrashBoxState.Stored.ContainsKey(boxId))
                    {
                        foreach (var kvp in TrashBoxState.Stored[boxId])
                        {
                            ...
                            if (SalesUnifiedFinal.WeightPerUnit != null && SalesUnifiedFinal.WeightPerUnit.TryGetValue(pid, out float weightOfSingleItem))
                            {
                                ...
                                // Weighted products: 1 kg = 1 Store Point.
                                storePointValue += kgSpoiled;
                            }
                            else
                            {
                                ...
                                // Regular products keep the old rule: 1 item = 1 Store Point.
                                storePointValue += qty;
                            }
                    ...
                    if (storePointValue > 0f && StoreLevelManager.Instance != null)
                    {
                        int pointsToAdd =
                            ExpirationStorePointLedger.ConsumeReward(storePointValue);

                        if (pointsToAdd > 0)
                            StoreLevelManager.Instance.AddPoint(pointsToAdd);

                        StatisticMod.Plugin.DebugLog(
                            $"[Punkty Sklepu] Kosz: produkty={totalItemsThrown}, " +
                            $"wartoscPunktowa={storePointValue:0.###}, dodano={pointsToAdd}, " +
                            $"resztaKg={ExpirationStorePointLedger.RewardRemainder:0.###}.");
                    }

                    // Persist the weighted Store Point fraction together with expiry data.
