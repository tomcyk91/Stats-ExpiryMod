...
namespace SmartExpiration.Patches
{
    public class BoxExpirationLabel : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            // Expiration text on a carton is visible only while the carton is open.
            // Closed cartons never show the floating expiry label.
            bool isOpen = false;
            try { isOpen = _box.IsOpen; } catch { }
            ...
            bool showDates =
                PluginConfig.ShowDatesOnBoxes != null &&
            ...
            if (playerTf != null)
            {
                ...
                // Keep visual labels local to the player.
            ...
            // The previous global updater intentionally showed the label only
            // on the box currently held by the player. That made the label
            // disappear immediately after placing the box on the floor/rack.
            // Nearby boxes now keep their expiry label visible.
            SetTextEnabled(true);
            ProcessLogicUpdate();
        }
