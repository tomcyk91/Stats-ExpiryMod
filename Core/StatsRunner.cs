...
namespace StatisticMod
{
    public class StatsRunner : MonoBehaviour
    {
        ...
        {
            VendingSalesTracker.ResetBaselines();
            try { SmartExpiration.Patches.DailySpoilagePatches.ResetCleanupGuard(); } catch { }
            if (_instance == null)
            {
            ...
            // A load/reload returns the game to its last committed save state.
            // Do not interpret the day jump as a naturally completed day.
            _instance._lastDay = -1;
            _instance._pendingDayChangeSave = false;
            _instance._saveDayChangeAfter = 0f;
            _instance._newGameResetPending = false;
            _instance._newGameResetDeadline = 0f;
            _instance._timer = 0f;
        ...
        void Update()
        {
            try
            {
                ...
                if (_pendingDayChangeSave && Time.realtimeSinceStartup >= _saveDayChangeAfter)
                {
                    ...
                    try { VendingSalesTracker.SampleNow(); CustomerBasketStore.SaveNow(); }
                    catch (Exception ex)
                    {
                        Plugin.DebugWarning("[StatisticMod] Delayed customer/checkout save failed: " + ex.Message);
                    }
                }
        ...
        {
            // Ordinary application exit is NOT a commit point. If the player exits
            // without saving, runtime stats must roll back together with the game.
            // Only finish a commit that was already triggered by a completed day.
            if (!_pendingDayChangeSave) return;

            try { StatsStore.SaveNow(); } catch { }
            try { BusinessAnalysisStore.SaveNow(); } catch { }
            try { DailySummaryStore.SaveNow(); } catch { }
            try { VendingSalesTracker.SampleNow(); CustomerBasketStore.SaveNow(); } catch { }
        }
    }
}
